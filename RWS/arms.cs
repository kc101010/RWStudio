    ...
    public partial class arms : Form
    {
        ...
        private void arms_Load(object sender, EventArgs e)
        {
            ...
            if (New_edit.lastleg != null)
            {
                ...
                {
                    if (txt[i].Tag != null)
                        txt[i].Text = data["arm_" + New_edit.lastleg][txt[i].Tag.ToString()];
                }
                for (int i = 0; i < cb.Count; i++)
                {
                    if (cb[i].Tag != null)
                        cb[i].Text = data["arm_" + New_edit.lastleg][cb[i].Tag.ToString()];
                }
                for (int i = 0; i < ch.Count; i++)
                {
                    if (ch[i].Tag != null)
                        ch[i].Checked = Convert.ToBoolean(data["arm_" + New_edit.lastleg][ch[i].Tag.ToString()]);
                }
