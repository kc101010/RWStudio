    ...
    public partial class legss : Form
    {
        ...
        private void arm_leg_Load(object sender, EventArgs e)
        {
            ...
            if (editUnit.lastleg != null)
            {
                ...
                {
                        if (txt[i].Tag != null)
                            txt[i].Text = data["leg_" + editUnit.lastleg][txt[i].Tag.ToString()];    
                }
                for (int i = 0; i < cb.Count; i++)
                {
                    if (cb[i].Tag != null)
                        cb[i].Text = data["leg_" + editUnit.lastleg][cb[i].Tag.ToString()];
                }
                for (int i = 0; i < ch.Count; i++)
                {
                    if (ch[i].Tag != null)
                        ch[i].Checked = Convert.ToBoolean(data["leg_" + editUnit.lastleg][ch[i].Tag.ToString()]);
                }
